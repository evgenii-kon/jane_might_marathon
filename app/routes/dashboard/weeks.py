...
async def week_detail(
    ...
    for lesson in lessons:
        is_started = await lesson_progress_service.is_lesson_started(
            current_user.id, lesson.id
        )
        is_completed = await lesson_progress_service.is_lesson_completed(
            current_user.id, lesson.id
        )

        # Получаем прогресс упражнений для урока
        exercise_progress_service = UserExerciseProgressService(db)
        exercise_progress = await exercise_progress_service.get_lesson_progress(
            current_user.id, lesson.id
        )
