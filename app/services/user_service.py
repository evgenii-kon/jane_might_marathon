...
class UserService:
    def __init__(self, db: AsyncSession):
        ...
        self.cache = CacheService(prefix="users", ttl=300)
    ...
    async def get_all_users(self) -> List[UserResponse]:
        ...
        if cached:
            return [UserResponse.model_validate(u) for u in cached]
    ...
    async def get_user_by_id(self, user_id: int) -> UserResponse:
        cached = await self.cache.get("id", user_id)
        if cached:
    ...
    async def get_user_by_email(self, user_email: str) -> UserResponse:
        cached = await self.cache.get("email", user_email)
        if cached:
            return UserResponse.model_validate(cached)
    ...
    async def get_user_by_name(self, user_name: str) -> UserResponse:
        cached = await self.cache.get("name", user_name)
        if cached:
            return UserResponse.model_validate(cached)
