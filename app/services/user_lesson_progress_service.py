...
class UserLessonProgressService:
    ...
    async def mark_lesson_as_completed(
        ...
        if await self.is_lesson_completed(user_id, lesson_id):
            raise HTTPException(400, f"Lesson {lesson_id} already completed")

        progress = await self.repository.mark_completed(user_id, lesson_id)
    ...
    async def mark_lesson_as_started(
        ...
        progress = await self.repository.mark_started(user_id, lesson_id)
        return UserLessonProgressResponse.model_validate(progress)

    async def is_lesson_started(self, user_id: int, lesson_id: int) -> bool:
        """Проверить, начат ли урок"""
        return await self.repository.is_started(user_id, lesson_id)

    async def is_lesson_completed(self, user_id: int, lesson_id: int) -> bool:
        """Проверить, пройден ли урок"""
        return await self.repository.is_completed(user_id, lesson_id)

    async def get_next_lesson_in_week(
        self, week_id: int, current_order: int
    ) -> Optional[LessonWithProgressResponse]:
        """Получить следующий урок в неделе"""
        lessons = await self.lesson_repository.get_by_week_id(week_id)
    ...
    async def get_user_total_progress(self, user_id: int) -> UserTotalProgressResponse:
        """Получить общий прогресс пользователя по урокам"""
        completed_lessons = await self.repository.get_completed_lesson_ids(user_id)
        all_lessons = await self.lesson_repository.get_all()
    ...
    async def get_week_progress(self, user_id: int, week_id: int) -> WeekProgressSummary:
        """Получить прогресс по урокам в конкретной неделе"""
        lessons = await self.lesson_repository.get_by_week_id(week_id)
        completed_count = await self.repository.get_completed_count_by_week(user_id, week_id)
    ...
    async def get_lessons_with_progress(
        ...
        lessons = await self.lesson_repository.get_by_week_id(week_id)
        completed_ids = await self.repository.get_completed_lesson_ids(user_id)
        started_ids = await self.repository.get_started_lesson_ids(user_id)
